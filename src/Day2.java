...
// in the sky. You gently land in a fluffy pile of leaves. It's quite cold, but you don't see much snow.
...
// Each time you play this game, he will hide a secret number of cubes of each color in the bag, and your goal is to figure out
...
// Each game is listed with its ID number (like the 11 in Game 11: ...) followed by a semicolon-separated list of subsets of cubes
...
public class Day2 {
    private static Map<Integer, List<Map<String, Integer>>> parseGames(String input) {
        Map<Integer, List<Map<String, Integer>>> games = new HashMap<>();

        String[] gamesStr = input.split("\n");

        for(String game : gamesStr) {
            String[] gameParts = game.split(":");
            int gameNo         = Integer.parseInt(gameParts[0].split("\s")[1]);
            String[] setParts  = gameParts[1].split(";");
            List<Map<String, Integer>> sets = new ArrayList<>();

            for (String set : setParts) {
                String[] cubeInfos           = set.split(",");
                Map<String, Integer> setInfo = new HashMap<>();

                for (String info : cubeInfos) {
                    String[] infoParts = info.trim().split("\s");
                    int colorCount     = Integer.parseInt(infoParts[0]);
                    String color       = infoParts[1];
                    setInfo.put(color, colorCount);
                }
                sets.add(setInfo);
            }
            games.put(gameNo, sets);
        }
        System.out.println(games);
        return games;
    }

    public static int getSum(Map<Integer, List<Map<String, Integer>>> games) {
        int sum= 0;
        for(Integer gameNo : games.keySet()) {
            boolean possible= true;
            List<Map<String, Integer>> sets = games.get(gameNo);
            for (Map<String, Integer> set : sets) {
                if ((set.get("red") != null && set.get("red") > 12) || (set.get("green") != null && set.get("green") > 13) || (set.get("blue") != null && set.get("blue") > 14)) {
                    possible = false;
                    break;
                }
            }
    ...
    public static int getSumPowerOfMinSet(Map<Integer, List<Map<String, Integer>>> games) {
        int sum = 0;
        for (List<Map<String, Integer>> sets : games.values()) {
            ...
            for(Map<String, Integer> set : sets) {
                for(Map.Entry<String, Integer> entry : set.entrySet()) {
                    ...
                    if(minCubes.containsKey(color)) {
                        ...
                        minCubes.put(color, count);
                    }
                }
            }
            int productOfMinSet = 1;
            for (int value : minCubes.values()) {
                productOfMinSet *= value;
            }
            sum += productOfMinSet;
        }
        return sum;
    ...
    public static void main(String[] args) {
        String input = Day1.getFileAsString("input/Day2");
        Map<Integer, List<Map<String, Integer>>> games = parseGames(input);
        System.out.println(getSum(games));
        System.out.println(getSumPowerOfMinSet(games));
    }
}
